...
class CalendarsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should NOT accept incomplete records' do
    post calendars_url,
         params: { calendar: { title: '', description: @calendar.description, start_time: @calendar.start_time,
  ...
  test 'should NOT accept empty description' do
    post calendars_url,
         params: { calendar: { title: @calendar.title, description: '', start_time: @calendar.start_time,
                               end_time: @calendar.end_time } }
    assert (400...500).include?(response.code.to_i)   # not ok
  end
  ...
  test 'should NOT accept empty start time' do
    post calendars_url,
         params: { calendar: { title: @calendar.title, description: @calendar.description, start_time: nil,
                               end_time: @calendar.end_time } }
    assert (400...500).include?(response.code.to_i)   # not ok
  end
  ...
  test 'should NOT accept start time in year 2023 or older' do # Code refrence for time: https://docs.ruby-lang.org/en/master/Time.html
    post calendars_url,
         params: { calendar: { title: @calendar.title, description: @calendar.description,
                               start_time: Time.new(2023, 12, 31, 10, 0, 0), end_time: @calendar.end_time } }
    assert (400...500).include?(response.code.to_i)   # not ok
  end
  ...
  test 'should NOT accept start time outside business hours' do
    post calendars_url,
         params: { calendar: { title: @calendar.title, description: @calendar.description,
                               start_time: Time.new(2024, 1, 1, 7, 0, 0), end_time: @calendar.end_time } }
    assert (400...500).include?(response.code.to_i)   # not ok
  end
